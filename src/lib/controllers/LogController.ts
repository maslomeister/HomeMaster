...
export class LogController {
   ...
   */
    ...
    PythonInterop.error(args.join(" "));
    throw new Error([`%c HomeMaster %c ERROR %c`, 'background: #ff6d05; color: black;', 'background: #c70808; color: black;', 'background: transparent;', ...args].join(' '));
  }
  ...
  static raiseError = debounce((...args: any[]) => {
    ...
    if (LogController.errorCounts[msg] <= errorToastLimit) PythonInterop.toast("HOME MASTER ERROR", msg);
